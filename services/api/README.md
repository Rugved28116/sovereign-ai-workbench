...
Stage executions and model-stage budget units have independent per-run limits, each configurable from 1 to a hard maximum of 16 (default 16). An oversized plan is rejected before any stage or provider execution. Before each coordinator call, the runner classifies the canonical stage type: all five currently supported types are model-backed; unknown types fail closed. It reserves one model-budget unit before entering the coordinator and never refunds it based on an executor's self-report. Thus under-reporting cannot bypass the limit; a routing or chaining failure inside the coordinator may consume a unit even when no provider call occurred. A rejection before the coordinator consumes none. `TaskRunResult.model_invocations` is this conservative reserved count, not independently observed provider telemetry. Runtime counters stop before the next stage when a limit is reached. `TaskRunResult` contains the latest immutable state, counts, status, and controlled message/code, but no model text. A limit can leave a **nonterminal** `RUNNING` state for an explicit later call; already-terminal inputs return zero additional counts. Current routed stages invoke a provider at most once; future non-model stage types will need their own trusted classification before admission.
