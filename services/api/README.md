...
This is bounded synchronous-in-order orchestration, not an open-ended autonomous loop. It has no durable checkpoint, public endpoint, active cancellation, retry, or cross-process idempotency. Callers must not execute the same task snapshot concurrently. If an unexpected coordinator fault prevents a confirmed replacement snapshot, the typed run error reports the last confirmed state but cannot prove whether the in-flight stage had side effects; callers must reconcile before any new attempt. Persistence and atomic stale-snapshot/idempotency enforcement remain future work. `/v1/generate` is unchanged.
