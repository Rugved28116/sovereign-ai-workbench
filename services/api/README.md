...
The pluggable `StageExecutor` contract returns an immutable `StageExecutionResult` containing a stage ID, terminal status, optional selected model ID and output reference, and controlled failure fields. `RoutedAgentStageExecutor` handles the current `generate`, `code`, `document`, `vision`, and `reason` stage labels: it routes each stage's exact capability set through the existing Stage 1/Stage 2 router, then calls the approved `ModelProvider` once. Each stage receives only the validated original task prompt; earlier outputs are **not** chained into later prompts. This does not add document/image payload handling or make unavailable capabilities routable. Valid UTF-8 model output is bounded to 1 MiB and represented in state by a `sha256:` integrity identifier. This is not a retrievable artifact reference: output content is not persisted or returned by this internal coordinator yet.
